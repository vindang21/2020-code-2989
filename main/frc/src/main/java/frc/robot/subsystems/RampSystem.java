...
public class RampSystem extends Subsystem {
  ...
  {
    leftMotor = new StarTalonSRX(RobotMap.RAMP_MOTOR_LEFT_PORT);
    rightMotor = new StarTalonSRX(RobotMap.RAMP_MOTOR_RIGHT_PORT);
  }
  ...
  private void applySpeedControllerPreferences() {
    leftMotor.setMaxOutputFunction(() -> RobotMap.RAMP_MOTOR_SPEED);
    rightMotor.setMaxOutputFunction(() -> RobotMap.RAMP_MOTOR_SPEED);
  }
  ...
  /**
    ...
    public void rampUp()
    {
        leftMotor.feed(); 
        rightMotor.feed();
        leftMotor.set(RobotMap.RAMP_MOTOR_SPEED);
        rightMotor.set(RobotMap.RAMP_MOTOR_SPEED);
    }//end intakeBall
}
